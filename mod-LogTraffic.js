exports.logReader = function (req, callback) {
    ...
    const formatTraffic = parsedUrl.query.formatTraffic || 'Type';
    const nextFormatTraffic = formatTraffic === 'Type' ? 'Date' : 'Type';
    ...
    rl.on('close', () => {
        let output = `
            ...
            <div id="traffic-section" class="section">
                <h2 id="traffic-header">Traffic</h2>
                <div id="traffic-buttons">
                    <button onclick="updateQueryParam('formatTraffic', '${nextFormatTraffic}')">Sort Traffic by ${nextFormatTraffic}</button>
                </div>
                <div id="traf-box" class="traf-container">
        `;

        if (formatTraffic === 'Type') {
